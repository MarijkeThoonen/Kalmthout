...
source(file.path(git_root, "MarijkeThoonen", "src","connect_to_access_dbi.R"))
...
con <- connect_to_access_dbi(
  db_file_path = file.path(git_root, "MarijkeThoonen", "data", 
                           "Heidebegrazing20112014.mdb")
  )

# om een lijst van tabellen in de databank te tonen:
dbListTables(con)
# om de kolomnamen van een tabel te kennen:
dbListFields(con, "tabelnaam")

# om een tabel uit de databank te halen:
tabelnaam <- dbReadTable(con, "tabelnaam")

# Om een SQL query binnen te halen:
res <- dbSendQuery(con, "SELECT * FROM tabelnaam WHERE kolomnaam = 4")
dbFetch(res)

# om de verbinding met de databank te verbreken:
dbDisconnect(con)
```
